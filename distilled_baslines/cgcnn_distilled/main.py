...
parser.add_argument('--pretrained_model', default="../model/crysgnn_state_checkpoint_v1.pth.tar",type=str, help='Pretrained Model Name')
...
def main():
    ...
    crysgnn_model = CrysGNN(orig_atom_fea_len, nbr_fea_len, atom_fea_len=args.atom_fea_len, n_conv=5)
    checkpoint_file_path = args.pretrained_model
    ...
    for epoch in range(args.start_epoch, args.epochs):
        # train for one epoch
        train(train_loader, model,crysgnn_model, criterion, optimizer, epoch, normalizer,args)
...
def train(train_loader, model,crysgnn_model, criterion, optimizer, epoch, normalizer,args):
    ...
    for i, (input, target, _) in enumerate(train_loader):
        ...
        _, _, _, _, bt_atom_fea = crysgnn_model(*input_var,args.cuda)

        loss_p = criterion(output, target_var)

        loss_emb = 0
        for k in range(len(bt_atom_fea)):
            loss_emb += criterion(atom_fea[k], bt_atom_fea[k])
        loss_emb = loss_emb / len(bt_atom_fea)

        loss = 0.5 * loss_p + 0.5 * loss_emb
